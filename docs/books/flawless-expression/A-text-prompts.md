...
### 用途
在开始正式写作前，强制生成带有逻辑校验和验收标准的章节骨架。拒绝流意识的堆砌，确保文章结构符合 [01-mindset.md](01-mindset.md) 中的逻辑闭环要求。
...
### 用途
在 RAG 场景强制模型只基于上下文回答，并把每句话绑定到来源标识；上下文不足时拒答。方法论与拒答口径参见 [02-facts.md](02-facts.md)。
...
### 输出契约
- 只输出答案正文（短句为主）。
- 每一句话句末必须追加来源标注，格式为 `(source:<id>)`。
- 若上下文不足，必须输出 `INFO_MISSING` 并给出缺口字段清单。
...
### 输出契约
- 只输出 JSON 字符串本体。
- 顶层字段固定：`decisions`（array）、`action_items`（array）、`risks`（array）。
- 每个行动项字段固定：`title`、`owner`、`deadline`、`priority`、`blockers`。
