...
## 0) 先骂后改：Prompt 质量控制最常见的假动作

- **把“写得更像人”当目标**：更像人不等于更可用，越像人越爱插话。改法：先锁输出协议，再谈措辞。
- **把“多试几次”当回归**：多试几次只能让你更熟悉运气。改法：把失败样本入库，改动前后同口径跑一遍再裁决。
- **把“模型版本升级”当优化**：升级只是换了一个随机数发生器，旧问题可能换皮继续。改法：先固化判定规则与回滚策略，再考虑换模型。
- **把图片当装饰**：插图有文字、水印、风格漂移，结果读者学不到、你也维护不了。改法：无文字底图 + 风格底座 + 留白叠字。
...
本仓库也提供一键门禁脚本：`bash skills/check_docs.sh`。
