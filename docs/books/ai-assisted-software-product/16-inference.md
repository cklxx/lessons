...
## 关键流程图（纯文本）：推理优化到回滚的闭环

```text
写取舍卡（质量/延迟/成本 + 守门指标 + 回滚）
  ↓
建立基线（同一批样本 + 同一口径 + 同一报告）
  ↓
选择优化（先便宜后昂贵）
  - prompt/上下文裁剪/缓存/并发控制
  - 再考虑引擎/量化/路由/更强硬件
  ↓
跑对比表（主指标 + 守门指标）
  - 未达标或守门越界：不发布，回滚到上一版本组合
  - 达标：进入灰度
  ↓
灰度发布（慢推进）
  - 线上守门越界：按表降级/熔断 → 一键回滚 → 留证据
  - 稳定：固化阈值与降级梯度
  ↓
回流与迭代
  - 把超时/高成本/坏体验样本写进回归集
...
线上出现成本尖峰或延迟/超时尖峰时，优先按 10 分钟止损 Runbook 执行（先降级/限流/回滚再定位）：见 [E-runbooks.md](E-runbooks.md)（RB-02/RB-03）。阈值三段式与告警到动作映射见 [F-metrics-alerts.md](F-metrics-alerts.md)。

## 推理三角形：先把取舍写清楚
![图 16-1：质量/延迟/成本三角形与守门指标示意](../../assets/figure_16_1_1765971418977.png)

文字版图 16-1：推理三角形与守门指标（不依赖图片也能裁决）

| 你想优化 | 常见动作 | 你必须守住 | 常见代价 |
| --- | --- | --- | --- |
| 质量 | 更强模型、更长上下文、更深检索、更复杂工具链 | 成本与延迟预算、错误与超时率 | 单次成本上升、P95 变差 |
| 延迟 | 缩上下文、减少工具、流式优先、并发控制 | 质量底线、拒答体验 | 命中率下降、解释性变差 |
| 成本 | 缓存、摘要、降级梯度、量化与更省的后端 | 质量门槛、合规与审计 | 需要更强门禁与回滚 |

守门指标是围住三角形的边：错误率、超时率、拒答率、预算耗尽率。一旦越界就触发降级或回滚，让取舍可裁决而不是靠吵架。
...
2) **钱到底花在了哪里？**  
成本不是抽象的模型很贵，而是可分解的：输入 token、输出 token、工具调用成本（按工具类型与计费单位拆分）、重试与回退、缓存命中率。只有当你能把账单分摊到这些可控变量上，预算与降级才不是拍脑袋。[6]
...
- 工具：`tool_name/tool_calls/tool_ms/tool_cost_units`（按工具类型拆分，而不是只数次数）；
...
可执行做法（从低成本到高成本）：
- 滑动窗口：只保留最近 N 轮对话，把历史变成摘要。
- 上下文预算：为系统提示、对话、检索证据、工具输出分别设置 token 上限，超额就触发摘要或降级。
- 触发式检索：只有当问题需要外部证据时才做 RAG，避免每次都检索与重排。

| 上下文块 | 建议上限 | 触发策略 |
| --- | --- | --- |
| 系统提示 | 固定 | 版本化（prompt_version） |
| 最近对话 | N 轮或 K tokens | 超额则摘要 |
| 历史摘要 | 1 段 | 仅在任务切换时刷新 |
| 检索证据 | M 段 | 仅在需要证据时注入 |
| 工具输出 | 只保留必要字段 | 过期或改成摘要 |
...
缓存 key 的最小配方（避免错用导致串号）：
- `model_version + prompt_version + normalized_input`
- 如果有 RAG：加上 `index_version + topk_doc_ids`
- 如果有工具：加上 `tool_name + tool_args_fingerprint + tool_version`

最小失效策略：版本变更即失效；敏感数据不入共享缓存；对缓存穿透加并发合并（避免 cache stampede）。
...
并发保护的最小集合：
- 设定 inflight 上限与排队超时（避免排队把 P95 拖爆）。
- 按租户/用户做配额与限流（避免单点把全局拖死）。
- 失败要可解释：返回 429/503 并给恢复指引，而不是默默超时。
...
文字版图 16-2：推理可控闭环（不依赖图片也能实现）

```text
预算与护栏
  → 并发控制（inflight/queue/timeout）
  → 缓存与复用（prompt/RAG/tool）
  → 降级梯度（更短上下文/更少工具/更省模型）
  → 门禁与回滚（守门越界即阻断）
  ↺ 指标回写到预算与护栏
```

| 环节 | 你输入什么 | 你做什么决策 | 你输出什么 | 你必须观测什么 |
| --- | --- | --- | --- | --- |
| 预算与护栏 | budget_ms、cost_budget | 允许继续还是先降级 | 目标策略 | 预算耗尽率 |
| 并发控制 | inflight、queue | 排队或拒绝 | 429/503 或继续 | 排队时长、P95 |
| 缓存与复用 | cache_key、ttl | 命中或穿透 | cached 或 miss | 命中率、穿透率 |
| 降级梯度 | 当前指标 | 选哪条省成本路径 | degrade_level | 降级次数、成功率 |
| 门禁与回滚 | 守门阈值 | 通过或阻断 | release/rollback | 错误率、超时率 |
...
## 示例（可复制）：一个能跑的最小推理网关（Python）：预算、缓存、并发上限
...
伪代码：`/chat` 的最小处理链路（把策略写成可执行逻辑）

```text
handle_chat(req):
  trace_id = req.trace_id
  if inflight_over_limit(req.tenant_id): return 429

  key = cache_key(model_version, prompt_version, normalized_input, index_version)
  if cache_hit(key): return cached_response

  with budget(req.budget_ms):
    resp = provider.generate(req.prompt)
    if timeout: resp = fallback_or_degrade()

  emit_metrics(trace_id, latency_ms, tokens_in, tokens_out, tool_cost_units, cached)
  cache_put(key, resp, ttl_s)
  return resp
```
...
```bash
python3 - <<'PY' > /tmp/inference_payload.json
import json
print(json.dumps(
...
curl -sS -X POST http://127.0.0.1:8787/chat \
  ...
  --data-binary @/tmp/inference_payload.json
```
...
### 量化决策卡：量化到底在推理三角形里优化哪一角

| 你的主要目标 | 你通常在优化什么 | 优先策略 | 你要验证什么 |
| --- | --- | --- | --- |
| 降单位成本 | 成本角 + 吞吐 | 先确认引擎支持，再做 4-bit 权重量化 | 业务回归集退化、P95 与单位成本 |
| 扩并发 | 延迟角（排队）+ 成本角 | 优先解决 KV Cache 与显存瓶颈，再考虑量化 | inflight 上限、OOM 率、排队时长 |
| 端侧可运行 | 成本角 | 量化与裁剪并用，优先可部署与可回退 | 端侧质量、耗电与温升 |

最小判断原则：
- 如果你的瓶颈在显存（KV Cache 或权重占用），量化更可能把收益兑现成并发与吞吐。
- 如果你的瓶颈在算力或网络，量化未必提速，先看 kernel 与批处理策略是否匹配。
- 任何量化都必须回到推理三角形：省出来的成本是否值得你承受的质量退化与工程复杂度。
- 强烈建议先在非核心路由或稳定模式试点量化，并保留 FP16/FP8 回退；量化一旦影响质量或无法兑现收益，应能快速回滚。
...
### 合规与审计：缓存、降级与日志的底线
- 缓存不跨租户共享，敏感数据不入共享缓存；明确 TTL 与清除策略。
- 日志与追踪避免落原文，优先记录摘要与指纹；保留 `model_version/prompt_version/index_version` 与降级决策证据，便于审计。
- 降级不等于更弱的安全：安全门禁与权限边界在任何降级层级都必须保持。
...
## 复现检查清单（本章最低门槛）
- 推理取舍卡已写：质量/延迟/成本/回滚清晰，且门槛能裁决是否发布。[6]
- 基线对比表可复跑：优化前/后同口径对比（质量、P95、成本与守门指标），并给出结论与回滚条件。[6]
- 降级梯度可触发：尖峰与越界时系统能自动止损、记录原因并可回滚到稳定模式。[6]

## 常见陷阱（失败样本）
1. **现象：** 优化了延迟，但质量下降，用户反而流失。  
   **根因：** 没有写清质量底线；缺少业务回归门禁，导致“更快但更错”。[6]  
   **复现：** 为了 TTFT/TPOT 降上下文/降检索/降工具链，回归集中准确率或引用覆盖明显下降，但仍继续发布。  
   **修复：** 先定质量门槛与回归集：任何延迟优化必须在同口径下证明质量不退化；不达标就回滚。[6]  
   **回归验证：** 固定回归集与对比表稳定复跑：延迟改善的同时，主指标达标且守门指标不越界。

2. **现象：** 成本越来越高，却说不清为什么；预算与账单像黑箱。  
   **根因：** 缺预算与审计字段；上下文长度、重试、工具调用成本不可见。[6]  
   **复现：** 线上单位成本持续上升，但无法拆分到 tokens/tool_calls/retry/cache_hit 等可控因子；只能凭感觉调参数。  
   **修复：** 把成本写进可观测字段与版本组合；设置任务级预算与降级梯度，越界自动降级/止损并留证据。[6]  
   **回归验证：** 在压力回归与真实分布下，单位成本与预算耗尽率稳定；越界时能按表降级并恢复到基线区间。

3. **现象：** 尖峰时系统崩溃，恢复慢；偶发慢请求拖死队列。  
   **根因：** 没有并发闸门、超时与降级策略；把稳定性寄托在“不会尖峰”。[6]  
   **复现：** 模拟并发上升或上游超时，inflight 持续堆积直至 OOM/雪崩；没有 429/熔断/降级层。  
   **修复：** 先设计并发闸门 + 超时预算 + 降级梯度；尖峰先保底（限流/降级/熔断）再恢复。[6]  
   **回归验证：** 压测与故障注入下，系统能稳定返回 429/降级结果且可恢复；关键指标越界会触发回滚并留证据包。
