...
!!! note "如何使用图 8-2"
    用这张图检查“输出区是否可解释、可恢复、可学习”：用户一眼能看到进度、证据与下一步，而不是只看到一段答案。
    - **上：进度**（检索/调用工具/生成中）+ 可取消/可重试，让等待可预期。
    - **中：结果**（可复制/可保存/可导出）+ 结论优先，让结果可采纳。
    - **下：依据**（引用卡片/工具摘要）+ 可折叠，让信任可建立。
    - **底：边界与反馈**（何时不可靠/需要补什么）+ 一键反馈，让失败能回流成样本。
