...
5. **部署形态对比**：导出 BF16/FP16 与 4/8-bit 量化版，记录吞吐、延迟与质量指标差异；推理配置与基准脚本建议放在你的工程仓库中（示例：`serve/`），并通过自动化任务（示例：`make infer-benchmark`）固化回归门槛。模型卡需注明许可证、训练数据来源与限制，降低合规风险。[45]
