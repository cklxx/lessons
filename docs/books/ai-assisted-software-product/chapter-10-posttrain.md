...
## 章节定位
本章解决“模型贴合业务、符合价值观”的问题。你将搭建低成本[后训练（Post-training）](glossary.md#post-training)流水线（含微调与偏好对齐），覆盖数据准备、训练、评估与安全审查。[40][42]
...
![图：后训练流水线总览](../../assets/fig-placeholder.svg)

*图 10-1：后训练流水线总览——失败样本→数据构建→SFT→偏好对齐→安全红队→评估回滚→发布（示意）*
<!-- TODO: replace with a diagram showing data flow + checkpoints + eval gates -->
...
**目标：** 把线上失败样本（越狱/注入/不礼貌/幻觉）整理为 SFT/偏好数据，并跑通最小对齐训练与安全回归，让每次“修复”都能回归验证。[40][42][51]
...
**约束：**
- 数据必须可追溯：来源、时间、脱敏策略写入 datasheet/审计记录。[41]
...
4. **训练与记录**：跑 LoRA/QLoRA SFT 或 DPO；记录 loss、偏好胜率、拒答率、显存与耗时。[40][42]
...
| 参数 | 作用 | 典型取舍 |
|---|---|---|
| `seq_len` | 上下文长度 | 越长越贵；先用 2k/4k 打底 |
| `micro_batch` | 单卡 batch | 显存受限时调小 |
| `grad_accum` | 梯度累计 | 用时间换显存 |
| `lora_r/alpha` | 适配容量 | r 越大越强但更贵 |
| `lr` | 学习率 | 过大易漂移，过小学不动 |
| `warmup` | 稳定训练 | 小数据集更敏感 |

![图：SFT 训练信号与早停策略（待补）](../../assets/fig-placeholder.svg)

*图 10-2：SFT 训练信号与早停策略——loss 曲线、验证集趋势与失败样本回写（示意）*
<!-- TODO: replace with your own loss curves for a real run -->
...
- rejected 是否具备“可学习差异”（不要太离谱也不要几乎一样）？
...
![图：安全回归与发布门禁（待补）](../../assets/fig-placeholder.svg)

*图 10-3：安全回归与发布门禁——红队集、阈值、回滚与豁免流程（示意）*
<!-- TODO: replace with your own gating diagram/runbook -->
...
2. **现象：** RLHF 训练不稳定，策略漂移，出现明显退化或“胡言乱语”。  
   ...
   **复现：** 观察 KL、reward 与样本输出，出现 abrupt shift 或崩溃。  
   **修复：** 动态调整 KL；收紧学习率与步数；提升奖励模型质量；必要时改用 DPO 走更稳路径。[41][42]  
   **回归验证：** 训练曲线稳定，评测表显示无灾难性退化；不稳定直接回滚配置。

3. **现象：** 红队攻击一打就穿透，模型泄露或越狱成功率高。  
   **根因：** 安全过滤缺失或未回归；把安全当“事后 patch”。[51]  
   **复现：** 跑固定攻击集，统计越狱/注入/隐私三类命中率。  
   **修复：** 把安全回归接入发布门禁；对高风险输出用“拒答 + 引导”模板；必要时回滚模型而不是硬扛。[51]  
   **回归验证：** 攻击集命中率下降到阈值以下；审计日志能追溯到触发样本与修复动作。

4. **现象：** 训练数据不可追溯，后来无法解释“为什么模型变了”。  
   **根因：** datasheet/审计缺失；失败样本与训练 run 没有关联。[41]  
...
下面把本章的训练与对齐流程抽象为可迁移原则，避免“只会跑通一次脚本”却无法长期迭代。
...
2. **高效微调成本表**：对比全参、LoRA、QLoRA 在 7B/13B 上的显存与小时成本，提供一个自动估算脚本（示例：`scripts/estimate_cost.py`）计算显存、训练时长与费用区间。推荐从“最小可行配置”起步（示例：7B + QLoRA + 10k SFT），并明确说明：能否在单卡 24GB 跑通取决于序列长度、batch、优化器、梯度累积与 checkpoint 等，需按硬件与目标调参。[39][47]
