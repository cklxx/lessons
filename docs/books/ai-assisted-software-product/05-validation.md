...
!!! note "如何使用图 5-2"
    此图把闭环从“进入→采纳”拆成可裁决的断点地图，用来快速排优先级：
    - **横轴**：闭环 3–5 个关键步骤。
    - **断点扫描**：每一步下方列出常见阻碍（入口不清、输入失败、等待焦虑、结果不可信、不知如何使用）。
    - **修复指引**：为每个断点明确恢复入口（示例/重试/追问/降级/联系支持）与证据口径，用于判断问题是否发生并验证修复效果。
...
除此之外，首次闭环还有一个更硬的要求：首个结果必须可采纳。对于 AI 产品，体验再顺滑，如果用户第一次拿到的是不可用结果（胡编、拒答却不解释、没有证据），闭环就是失败。

把它写成可裁决的门槛，通常只需要两条：
- 首次任务完成率达到门槛（例如 ≥ 基线 + 20pp）。
- 首次结果采纳率达到门槛（例如 ≥ 60%，且无引用强答率低于阈值）。

如果你只能在首版做一件打磨，优先做起步容易、过程可预期、结果可采纳三者之一。它们往往比新增功能更能提高闭环发生率。[4][6]
...
| 用户编号 | Persona 是否匹配 | 数据就绪度 | 任务类型 | 使用频次预期 | 反馈渠道 | 是否完成闭环（Y/N） |
| --- | --- | --- | --- | --- | --- | --- |
| U01 | 是/否 | 有可用数据/无数据/权限受限 | 例如：写周报 | 每周 1 次 | 群/表单/电话 |  |
...
对 AI 产品，样本同质性还要额外多一个维度：数据准备度。比如做文档问答/RAG 时，招募没有任何可上传资料或权限无法提供资料的用户，得到的大多是噪声而不是验证。
...
| 行为 | 首次结果采纳率 | 首次会话中被复制/保存/继续推进的比例 | ≥ 60% | < 40% |
| 行为 | 纠错成功率 | 用户触发纠错后仍能完成闭环的比例 | ≥ 50% | 连续下降 |
| 守门 | 无引用强答率 | 需要证据的问题中，无引用却给结论的比例 | ≤ 1% | 任一高危样本出现 |
| 守门 | 人工接管率 | 需要人工介入/联系客服才能完成闭环的比例 | ≤ 10% | 连续上升 |
| 守门 | 单次成本 | 平均/分位数 token 成本 | ≤ Z | 超预算即回滚 |
| 守门 | 端到端延迟 | P95 响应时间 | ≤ 基线 × 1.2 | > 基线 × 1.3 |
...
| 版本与口径 | 模型版本/提示词版本/检索参数/随机性控制（温度/采样） |
| 对照与复跑 | 改动前后复跑同一组场景脚本与失败样本集 |

![图 5-3：迭代卡片 + 对比表 + 裁决](../../assets/figure_05_3_iteration_card_loop_1766374251126.png)

!!! note "如何使用图 5-3"
    此图把卡片、对比与裁决串成闭环，用于标准化迭代决策：
    - **左侧（迭代卡片）**：确认本次迭代的假设、成功门槛、回滚策略与证据位置。
    - **中间（对比表）**：同口径对比改动前后，并检查守门指标是否越界。
    - **右侧（裁决）**：做出保留/继续/回滚的决定，并把特例沉淀为回归样本，防止未来退化。
...
- **周二–周三：改动**：只做能影响该断点的最小补丁；避免把顺手优化塞进同一轮。若涉及 prompt/RAG 调参，先用离线场景脚本做小时级复跑，过门槛再上小流量。
...
## 把 AI 的不确定性写进验证（否则无法裁决）
AI 参与闭环后，最大的新增变量不是功能，而是输出的不确定性。要把它纳入可裁决迭代，至少要做到两点：

1) 固定口径：同一组场景脚本、同一组失败样本、同一套模型与提示词版本，才能比较改动前后差异。  
2) 分离原因：区分是模型能力不足、检索证据不足、还是产品交互设计导致用户输入不足。

一个简单的分离法是三段复跑：
- 固定输入，重复跑 3 次，观察方差（估计模型抖动）。
- 固定模型与提示词，替换检索配置，观察证据变化（估计检索问题）。
- 固定后端输出，替换前端引导/示例，观察用户输入质量变化（估计交互问题）。
...
可以用下面这个最小模板，强制把 AI 特有失败模式写进去（证据不足、无引用强答、纠错无入口）：
...
输入：脱敏样例（含数据就绪度）
期望输出：结果结构 + 引用/证据要求
门槛：主指标 + 守门指标（例如首次采纳率、无引用强答率）
失败判定：哪些现象算失败（含拒答、胡编、超时、预算超限）
恢复入口：追问/补信息/降级/人工接管
回滚：如何一键切回保守模式
```
...
```text
核心闭环（3–5 步） -> 断点地图（触发条件/恢复入口） -> 指标树（NSM/关键行为/守门）
  -> 迭代卡片（门槛/失败判定/回滚/证据路径） -> 小补丁改动 -> 小流量观察
    -> 对比表 -> 裁决（保留/继续/回滚） -> 更新回归与断点地图
```

## 示例（可复制）：一周只修一个断点（并留下证据）

**目标：** 用一周把一个高频断点修到可裁决，并产出可归档证据（对比表 + 决策记录）。

**前置条件：**
- 你能描述核心闭环 3–5 步，并能定位一个断点（例如首次任务卡在证据不足，用户不知道怎么补信息）。
- 你愿意一周只改一个断点，拒绝顺手优化。[4]

**输出格式：**
- 迭代卡片：`docs/iteration/itr-YYYY-MM-DD.md`
- 对比表：`reports/iteration/itr-YYYY-MM-DD/compare.md`
- 失败样本：`reports/iteration/itr-YYYY-MM-DD/failures.jsonl`

**步骤：**
1. 写迭代卡片：目标断点、门槛（主指标）与守门指标、失败判定、回滚动作与证据路径。[6]
2. 固定 10 条失败样本：每条包含最小输入、期望行为、失败原因；先脱敏，再入库。[18]
3. 只做能影响该断点的最小改动：提示文案、追问逻辑、恢复入口、默认降级策略之一。
4. 小流量复跑同一批样本：生成对比表（改动前/后），并做裁决（保留/回滚/继续）。[6]
...
cards = sorted(Path('docs/iteration').glob('itr-*.md'))
if not cards:
  raise SystemExit('missing iteration card under docs/iteration/')
...
**失败判定：**
- 对比表不存在，或门槛与守门指标口径写不清。
- 断点修复后主指标没提升且守门退化（成本/延迟/错误率越界）。[6]

**回滚：**
- 回到上一稳定策略与版本组合；把触发样本写入阻断级回归，直到复跑通过再继续迭代。[6]

## 复现检查清单（本章最低门槛）
- 核心闭环可复跑：闭环 3–5 步清晰，并列出至少 5 个断点（触发条件、代价、恢复入口、验收方式）。  
- 指标树可裁决：NSM/关键行为/守门指标齐全，且口径（采集方式、时间窗、去重规则）写清楚。[6]
- 迭代卡片可审查：至少 3 张迭代卡片包含门槛、失败判定、回滚动作与证据路径（参考附录 A 的 A.7）。[4]
- 复跑有证据：一周内至少 5 次闭环复跑记录 + 一页改动前后对比表，并给出裁决（继续/回滚/暂停）。[6]

## 常见陷阱（失败样本）
1. **现象：** 有人说不错，但没有持续使用。  
   **根因：** 验证只收集观点，没有观察闭环是否发生；缺少断点地图与恢复入口。  
   **复现：** 用户能说出想法，但在无指导情况下仍无法完成一次闭环，或失败后只能放弃。  
   **修复：** 用任务驱动验证：盯完成一次闭环的证据；为每个断点补恢复入口与验收方式。[4]  
   **回归验证：** 一周内复跑同一任务脚本，闭环完成率上升，且失败后恢复率提升（有可观察证据）。[6]

2. **现象：** 越打磨越累，效果不明显。  
   **根因：** 没有门槛与止损线，优化对象漂移；把细节精修当成进度。  
   **复现：** 一周改了很多 UI/文案/提示，但说不清主指标变了多少，守门指标是否退化。  
   **修复：** 每轮只改一个断点，并提前写门槛、失败判定与回滚；无提升就回滚。[6]  
   **回归验证：** 每次合并都有对比表与裁决；没有证据的改动不会进入主干。[6]

3. **现象：** 指标上去了，但成本/风险爆了。  
   **根因：** 只看 NSM，不设守门指标与越界动作；把短期增长当长期可持续。  
   **复现：** 采纳率提升，但 token 成本、延迟或越权风险显著上升，且没有自动止损。  
   **修复：** 把成本/延迟/错误率/风险写成硬门槛；越界即降级、限流或回滚，并留痕。[6]  
   **回归验证：** 守门指标在灰度与全量阶段都不退化；越界事件能触发止损并产出证据包。[6]
