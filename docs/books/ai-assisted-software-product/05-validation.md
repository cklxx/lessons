...
!!! note "作图提示（建议画成步骤→断点→修复方向）"
    - 横轴：闭环 3–5 步（从进入到采纳）
    - 每一步下面标 2–3 个常见断点（入口不清/输入失败/等待焦虑/结果不可信/不知道怎么用）
    - 每个断点标注恢复入口（示例/重试/追问/降级/联系支持）与证据口径（你怎么知道它发生了）

你可以把上面这张表当作“断点地图”：每一步至少写一个常见失败点，并明确恢复入口与证据口径（你怎么知道它发生过）。一旦你写清楚断点，后面的打磨优先级就会变得非常朴素：先修最容易断的那一段。
