...
- 一份 Runbook 模板：事故发生时你按表执行，而不是凭情绪决策（Runbook 库见：[E-runbooks.md](E-runbooks.md)）。[5]
...
证据包建议统一目录与字段：`reports/YYYY-MM-DD/<change-id>/`（版本组合 + 门禁输出 + diff + 回滚指针），模板见：[附录 D：证据包与门禁速查](D-evidence-pack.md)。
...
指标字典、阈值三段式（基线分位数 + 倍数 + 绝对红线）与“告警→动作→Runbook”映射表见：[F-metrics-alerts.md](F-metrics-alerts.md)。
