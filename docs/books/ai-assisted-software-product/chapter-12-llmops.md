...
![图：LLMOps 运行闭环](../../assets/fig-placeholder.svg)

*图 12-1：LLMOps 运行闭环——评估（离线/在线）× 观测（成本/延迟）× 安全（注入/越狱）× 反馈（训练/配置回滚）（示意）*
<!-- TODO: replace with a diagram connecting eval/observe/guardrail/feedback loops -->
...
5. **门禁与告警**：门禁用于阻断发布；告警用于提醒排障；两者阈值分别校准并写清回滚动作。[6]
...
![图：反馈闭环（训练/提示/配置）](../../assets/fig-placeholder.svg)

*图 12-3：反馈闭环——负反馈与失败样本→对齐训练（第 10 章）/检索与切分（第 6 章）/推理配置（第 11 章）（示意）*
<!-- TODO: replace with a diagram showing how incidents feed back into training/config -->
