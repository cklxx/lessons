...
运行前先做一次健康检查：用你准备在 `--model` 里传入的同一个模型 id 运行 `gemini --model gemini-2.5-flash -o text 'ping'`（示例），确保能得到正常输出；如果失败，先补齐 `gemini` CLI 的认证信息（例如 API Key 或登录凭据），再继续。
