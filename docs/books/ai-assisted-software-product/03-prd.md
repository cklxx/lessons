...
!!! note "如何使用图 3-4"
    请按照此管道建立**从条款到门禁的可追溯机制**：
    - **源头控制**：在 PRD 中严格使用 MUST/SHOULD/MAY 定义条款。
    - **契约锁定**：将字段、错误码、鉴权等要求落实为 OpenAPI 契约。
    - **规则校验**：配置 Spectral 规则集，自动检查语法、一致性与产品合同。
    - **CI 拦截**：在流水线中拦截破坏性变更，并直接在 PR 评论中给出人类可读的修改建议。
...
!!! note "如何使用图 3-2"
    此卡片用于**快速评审与对齐**，确保 PRD 结构简单且可验收：
    - **愿景层**：确认一句话愿景与目标用户是否清晰。
    - **范围层**：对照检查“目标”与“非目标”，防止需求范围漂移。
    - **质量层**：量化 NFR（延迟/成本/风险）作为守门指标。
    - **兜底层**：明确验收门槛，并预设“触发条件 → 动作”的回滚开关。
...
!!! note "如何使用图 3-3"
    请使用此映射表进行**端到端的验收与证据留存**：
    - **列 1 验收条目**：从用户任务视角定义 PRD 验收标准。
    - **列 2 前端交互**：确保 UI 状态与文案让用户能理解、能行动。
    - **列 3 系统证据**：埋设关联的日志字段、指标或 Trace ID，证明功能按预期执行。
    - **列 4 降级策略**：预演异常情况，定义系统越界时的回滚或降级动作。
...
## 变更协议：让合同在迭代中存活
敏捷不是不写合同，而是构建一套低摩擦的合同修订机制。变更协议的作用是防止“文档腐烂”和“隐形债务”：任何影响范围、门槛、守门指标或回滚策略的改动，都必须遵循“文档即代码”原则，以原子化提交的方式同步更新。

### 变更分级与同步矩阵
变更不仅仅是代码修改，更是对系统承诺的重订。不同类型的变更需要不同维度的“合同”背书：

| 变更类型 | 必须同步更新的合同字段（PRD/Spec） | 必须补齐的门禁/证据 | 默认回滚动作 |
| :--- | :--- | :--- | :--- |
| **功能修正**（Fix/Tweak） | 验收标准细节、边界值定义 | 新增/更新测试用例 + 可复核证据（截图/日志） | 代码回退到上一稳定版本 |
| **逻辑变更**（Refactor/Change） | 核心业务流程图、策略/Prompt 约束 | 更新后的回归（E2E/对比表）通过报告 | 关闭/降级相关能力（Feature Flag/路由降级） |
| **破坏性变更**（Breaking） | API 契约、数据 Schema、下游兼容性声明 | 消费者契约测试 + 影子流量/灰度验证 | 停止扩量并回到旧版本；如涉及数据迁移，需配套补偿/修复脚本 |
| **性能/非功能**（NFR） | 延迟/成本阈值、并发限制、资源配额与越界动作 | 压测与指标对比（Baseline vs New） | 熔断降级/回滚配置（优先恢复稳定口径） |

### PRD Diff Checklist（最小变更清单）
每次修改 PRD，至少回答以下问题（缺一项，变更不得进入实现）：

- **范围**：目标/非目标是否变化？是否新增“隐性承诺”？
- **验收**：验收标准是否仍可映射到证据（测试/日志/指标/对比表）？
- **异常流**：是否新增失败路径？对应的用户恢复入口与系统补偿是否写清？
- **NFR/预算**：守门指标阈值是否变化？越界动作是否更新？
- **回滚**：回滚是否仍可执行？触发条件是否可观测？

### 原子化提交规范
为了确保“实现与合同永远一致”，执行严格的原子化提交策略：

1. **入口唯一**：变更必须始于一张变更卡片（Why/What/Gate/Fail/Rollback/Evidence），明确动机与范围。
2. **门槛先行**：先改门槛与失败判定（测试/脚本/验收条款），再改实现；否则会变成做完再找理由。
3. **同 PR 原子提交**：改动实现的 PR 必须同时更新 PRD 与验收门禁；只改实现不改合同视为未完成。
4. **证据留档**：每次合并必须产出对比表/回归报告并给出路径；没有证据就当没有改进。
...
## 变更协议追踪
- 变更卡片：<路径>
- 合同修订：<PRD/Spec 路径 + 变更摘要>
- 变更类型：Fix / Change / Breaking / NFR
- 验收证据：<测试/日志/指标/对比表 路径>
- 守门指标：<不退化项 + 阈值>
- 回滚动作：<开关/配置/版本回退 + 触发条件>
```
