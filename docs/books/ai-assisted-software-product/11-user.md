...
在界面上，你可以把“成员”和“权限”做成一个可解释的两栏结构，让拒绝与变更都有出处：
...
把邀请流程写成“可复盘的闭环”，你就能避免大多数协作启动失败：

```text
邀请 → 接受 → 加入 → 首登/绑定 → 权限生效 → 审计落盘
  ├─ 过期：重新发送
  ├─ 撤销：邀请失效并记录审计
  ├─ 重复邀请/成员已存在：提示切换为“调整权限”
  └─ 风险：默认最小权限；跨租户邀请必须阻断
```
