...
你可以将用户流拆解为三段（进入/执行/交付），并在每段中标出系统提示、用户可动作、失败恢复入口以及证据/日志点。为了识别风险，建议用红色标记高风险的不确定点，用绿色标记用户的自救出口。[71][72]
...
!!! note "如何使用图 4-2"
    此图展示了如何将原型截图与断点列表结合，以便于复盘与派发任务：
    - **左侧**：放置一张关键页面截图（低保真即可）。
    - **右侧**：列出断点列表（包含断点 ID、触发条件、用户行为、系统应给的恢复入口）。
    - **关联**：用编号将截图上的卡点位置与断点列表对应起来，确保每个问题都可被定位与追踪。
...
!!! note "如何使用图 4-3"
    此图展示了如何通过三块区域分别解决用户的起步、引导与失败恢复问题：
    - **空状态**：明确告诉用户这里能做什么。
    - **示例区**：提供 1–3 个可直接点击或复制的示例任务，降低启动门槛。
    - **失败恢复**：显式放出重试、查看示例或联系支持等入口，避免用户无路可走。
