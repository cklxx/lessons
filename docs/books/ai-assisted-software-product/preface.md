...
![图 P-1：价值/交付/治理三圈相交，中心为 AI Accelerator，并标注证据阈值、回滚阈值与审计阈值（示意）](../../assets/preface_p1_super_individual_1766373599659.png)

把这张图当作三条“默认阈值”来用：
- **价值的证据阈值**：没有可回放证据（原话/数据/对比表），就不进入实现。
- **交付的回滚阈值**：守门指标越界或关键失败可复现，就必须降级/暂停/回滚。
- **治理的审计阈值**：关键动作无法回答“谁/何时/对什么/做了什么/结果如何”，就不允许扩量。
...
![图 P-2：三阶台阶从 Caller→Designer→Shaper，分别交付 Demo、指标与护栏、权重与基准（示意）](../../assets/preface-super-individual-evolution.png)
