...
> PRD 写得再漂亮，如果实现时理解不一致，文档价值就会大打折扣。本章介绍如何用“可机读 + 可校验”的方式减少歧义。[11][12]
...
## 方法论速览
1. **先定结构再生成：** 用 YAML/JSON 固定 PRD 字段，生成后立刻做 Schema 校验；缺字段就算失败。[11]
2. **先写用例再画图：** 用例清单必须包含失败/补偿路径，然后再转流程/时序图；只画成功路径容易遗漏隐患。[12]
3. **先写访问模式再建表：** 先说清楚“怎么查/怎么写/多大流量”，再生成 Schema 与索引；脱离预算谈“性能设计”没有意义。[13]

![图 2-1：PRD 到 Schema 流水线](../../assets/ch02-prd-schema-pipeline.png)

## 典型场景
- **MVP 规划：** 快速完成登录、支付、内容推荐的全链路 PRD 草稿并自动校验必填字段。
- **性能先行：** 针对高并发接口，提前给出分区键、限流策略和幂等设计，避免上线后返工。

## 实战路径
```text
结构化需求上下文 → 结构化 PRD（YAML）→ Schema 校验 → 图表（时序/流程）→ OpenAPI → Schema/基准 → 评审与迭代
```
...
- 命名：PRD 必须包含 `revision` 字段，并在提交信息/变更记录里链接到决策编号。[4]
...
1. **现象：** PRD 看起来完整，但实现阶段争论不断，大家各写各的。  
   ...
   **复现：** 对一个核心链路（如登录或扣费）强制补全“超时/重试/退款/幂等”等异常场景，通常会发现缺失。  
...
3. **现象：** PRD/图表/契约与代码逐渐脱节，最终导致文档失去信任。  
