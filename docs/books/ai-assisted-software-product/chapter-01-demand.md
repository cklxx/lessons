...
2. **语义建模：** 结合情感/立场分类（例如基于 BERT 的微调模型）与主题建模（LDA/BERTopic）提炼高频痛点，形成可度量的需求分布。[8][9][26]
...
## 正文扩展稿（用于成书排版）
1. **场景化起手式：** 从一条用户投诉开始——用正反两种假设框定问题，再让代理爬取 1,000 条相似评论验证是否具备“共性痛点”。以混合检索（例如 BM25 + 向量相似度）定位语义近邻，拒绝凭感觉扩写需求。[7][27]
