...
产品假设（值得做吗？）
  ...
  → 主题×情绪（痛点分布 + 强度）
  → 问题—证据矩阵（样本/反例/不确定性）
  → 虚拟访谈（Persona 轮询，收集反例）
  → 七天实验门槛（点击/转化/留存 proxy）
  → 决策白板（保留/放弃/延期）→ 回到“假设”
```

*图 1-1：需求验证闭环流程图：从评论样本采集到商业实验门槛（纯文本示意）*
...
| 主题（痛点） | 样本量 n | 平均评分 | 负面占比 | 不确定性/反例（示例） |
|---|---:|---:|---:|---|
| onboarding 复杂 | 128 | 2.1 | 72% | 仅来自 App Store；需补论坛/工单样本 |
| 价格太贵 | 96 | 2.4 | 65% | 可能是区域定价/竞品促销驱动 |
| 性能卡顿 | 74 | 1.9 | 80% | 高配机不复现；需分设备/版本统计 |

*图 1-2：主题 × 满意度（关联表示例；可据此绘制热力图；务必标注样本量与时间窗口以规避伪相关）*
