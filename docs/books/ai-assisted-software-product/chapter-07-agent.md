...
> 让模型从“仅问答”进化为“自主查证、执行任务并留存确凿证据”，并且不越权、不乱用工具。[29][30][31]
...
3. **协作与仲裁：** 多 Agent 协作时，引入裁决/仲裁角色，防止陷入自我强化的回声室效应或死循环。[30]

## 实战路径
Agent 的本质是一个“状态机 + 工具路由器”。在进入代码前，先把数据流画清楚：输入从哪来、工具能做什么、如何记录证据、如何停止。否则你将迅速陷入“Demo 易得，生产难上线”的困境：它会在生产里以更高速度放大风险。[29][31]
...
如果你不写停止条件，Agent 会把“继续”当默认；生产事故通常来自“无限循环 + 真实副作用”。将预算参数配置化，避免硬编码：[29][30]
...
- **冲突仲裁**：Writer 与 Critic 冲突时，以“证据、引用或可验证检查”为裁决依据，而非依赖语气强弱。
...
4. **基准任务说话**：用固定任务集比较“纯 LLM vs Agent”的增益，并将失败样本反馈至提示词与工具设计迭代中，而非依赖主观体验。[50]
