...
prompt=$(cat <<'EOF'
你是中文技术书编辑。请检查以下章节是否缺少：章节定位/你将收获什么/方法论速览/实战路径/复现检查清单/常见陷阱/延伸练习/交付物清单与验收标准/深度解析：核心原则。
...
prompt=$(cat <<'EOF'
你是中文技术书写作者。请基于下面的小节内容，补一个“示例（可复制）”，必须包含：目标/前置条件/上下文/约束/输出格式/步骤/验证命令/失败判定/回滚。
...
### 2.7 生成“资料笔记大纲”（一条资料一页）
```bash
target_ref="[47] QLoRA"
prompt=$(cat <<'EOF'
你是中文技术书编辑 + 资深工程实践者。

我在整理《AI 辅助软件产品》的“参考资料笔记”，目标是：一条资料一页，读者看完就知道“这份资料讲什么、在本书里怎么用、最常见的误用是什么”。

请为下面资料输出一份可直接粘贴进 Markdown 的笔记草稿，必须包含：
- 资料类型（论文/标准/官方文档/书）
- 原始来源 URL（如果未知就留空）
- 对应章节（用“第 X 章”写）
- 一句话
...
## 3) 采纳规则（作者作为裁判）
- 只采纳能落到“可验收改动”的建议（新增示例、补步骤、改标题、补约束、补验收标准、补失败判定）。
