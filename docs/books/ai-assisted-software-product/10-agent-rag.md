...
## 章节定位
本章位于“全栈底座”之后，开始进入“智能层”。它不追求把你带进某个框架的细节，而是把智能层写成可治理系统：
...
- 一张“选型决策表”：面对不同问题，先选 RAG、Agent 还是别做“智能”。[6]
...
你会发现：只要评测与审计缺失，任何“优化”都会变成不可解释的漂移。[6]
...
| 你遇到的问题 | 更可能的解法 | 为什么 |
| --- | --- | --- |
| 回答缺事实、编造引用 | RAG | 缺“可检索证据”，先补证据链 |
| 需要调用外部系统完成任务 | Agent | 缺“行动能力”，先定工具边界 |
| 输出不稳定、风格漂移 | 评测与回归先行 | 没有基线就无法优化 |
| 成本与延迟失控 | 预算与降级先行 | 先止损，再谈提升 |
| 用户不信任、不敢用 | 证据与边界先行 | 让系统“可解释、可拒绝” |
...
2. **现象**：Agent “很勤奋”，但成本飙升且偶尔越权。  
   **根因**：没有预算/停止条件与工具边界；把“会做事”当成“该做事”。  
...
## 下一章
智能层之后，你会进入“产品模块”的世界：用户、权限、计费、数据等，把商业闭环与风险边界落到系统默认属性。下一章见：[`11-user.md`](11-user.md)。
