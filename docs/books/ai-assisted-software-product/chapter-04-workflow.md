...
> AI 能让你写得更快，也能让你更快地写错。本章核心原则：以测试与门禁约束“速度”，以审查与回滚保障“稳健”。[18][5][6]
...
4. **视 AI 产出为不可信代码：** 默认必须经过人类复核；对外部贡献/不熟悉模块尤其如此。[19]
...
建议将 Prompt 纳入版本控制（Code as Configuration），避免因“提示词差异”导致的结果漂移：
...
**步骤：**
1. 向 AI 提供函数签名与最小实现片段（控制上下文长度），并明确“禁止改实现，只能新增测试”。
...
- 仅在指标显著提升时才宣称优化；若出现性能回退，必须在合并前阐明原因。[6]
...
1. **现象：** AI 说“没问题”，但 CI 一跑就红，反复来回。  
   **根因：** 误将模型视作裁判，而非以 CI 为准绳；未能通过可执行的验证将争论转化为结论。[18]  
