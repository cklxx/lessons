...
>
> 说明：本仓库以文档为主，但 `skills/` 提供少量可复用脚本（例如一键门禁、自检、批量改写）。文中提到的 `make gate` 等任务名只是“可运行门禁”的形态示例，你可以用任意任务编排工具替代。
...
- **把讨论当交付**：争论很激烈、结论很确定、证据包为空 → 下次继续争。改法：决策必须绑定证据与止损线，不留证据的结论默认无效。
...
### 2.0 证据的六个硬字段（少一个都不算门禁）
很多团队“有清单但没门禁”，根因是门禁描述缺字段：能说服人，但不能被机器执行、不能被审计复跑。建议把每条门禁都写成 6 个字段：

- **目的**：这条门禁要防什么（退化/越权/泄露/成本失控/错配）。
- **入口**：怎么跑（命令/任务/脚本入口，至少能一键复跑）。
- **判定**：怎么判失败（阈值/退出码/对比规则，必须可证伪）。
- **证据**：输出在哪里（固定路径 + 固定文件名/格式）。
- **责任**：谁能裁决（owner/值班/审批人）。
- **回滚**：失败怎么退（回滚到哪个版本组合 + 如何证明已恢复）。
...
建议把复现包落到一个固定目录（示例：`reports/YYYY-MM-DD/<change-id>/`），保证每轮对比都能找到证据。更完整的结构与字段模板见：[附录 D：证据包与门禁速查](D-evidence-pack.md)。
常见线上事故的 10 分钟止损动作库见：[附录 E：10 分钟止损 Runbook 库](E-runbooks.md)。
指标字典、阈值三段式与告警到动作映射见：[附录 F：指标字典与告警门禁速查](F-metrics-alerts.md)。
...
- [第 17 章](17-deployment.md)：发布可追溯到版本组合；灰度推进/暂停/回滚按表执行；Runbook 10 分钟止损可演练（见 [E-runbooks.md](E-runbooks.md)）。
...
- **本仓库一键门禁**：`bash skills/check_docs.sh`（包含引用检查 + 严格构建）。
