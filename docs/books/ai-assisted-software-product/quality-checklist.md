...
> 用于审稿与自检的统一质量门禁文档，覆盖需求验证到部署运维的所有章节。作为**实施模板**，旨在协助将书中的方法转化为项目里的可执行检查。注意：本仓库仅包含文档，不提供这些 `make` 任务/脚本；你需要在自己的工程仓库中实现或用任意任务编排工具替代。
...
## 第一篇 指挥官
- **需求验证（Ch01）：** 推荐自动化任务（如：`make demand-benchmark`）
  - 主题覆盖度、标注一致性/混淆矩阵等质量指标达标（示例：F1 ≥ 0.8）；数据源与采集时间记录于数据卡片。
  - 生成《决策白板》：假设/证据/反例/风险/下周实验。
- **PRD/架构（Ch02）：** 推荐自动化任务（如：`make prd-validate`）
  - PRD 模板字段齐全（愿景、用户故事、埋点、异常路径、SLO）。
  - Mermaid 流程/时序图在 CI 通过渲染校验；SQL Schema 通过约束检查（索引、租户隔离、审计字段）。
- **UI/UX（Ch03）：** 推荐自动化任务（如：`make ui-regression`）
  - 视觉回归对比基线差异低于阈值（示例：< 0.5%）；可访问性（a11y）检查通过；多设备截图归档。

## 第二篇 工程师
- **编码工作流（Ch04）：** 推荐自动化任务（如：`make test`）
  - 覆盖率阈值（例如 80%）达标；pre-commit、SAST、依赖漏洞扫描无阻塞。
- **后端模块（Ch05）：** 推荐自动化任务（如：`make api-test`）
  - 身份验证、RBAC、速率限制与 Webhook 幂等性单测通过；开放接口生成最新 OpenAPI 文档。

## 第三篇 架构师
- **RAG（Ch06）：** 推荐自动化任务（如：`make rag-eval`）
  - RAGAS `Context Precision`、`Answer Relevance` ≥ 基线；嵌入/检索/重排序参数记录。
- **Agent（Ch07）：** 推荐自动化任务（如：`make agent-eval`）
  - ReAct/流程编排用例通过；工具调用白名单与速率限制日志齐全；长链路超时与重试策略验证。

## 第四篇 造物主
- **数据工程（Ch08）：** 推荐自动化任务（如：`make data-clean`）
  - 去重、有害内容过滤（即内容无害化处理）、PII 过滤报告；合成数据与真实数据分层质量评分。
- **预训练/增量（Ch09）：** 推荐自动化任务（如：`make pretrain-eval`）
  - Tokenizer/语料采样报告；显存/吞吐/成本估算表与实验日志。
- **后训练（Ch10）：** 推荐自动化任务（如：`make sft-train && make dpo-train`）
  - Loss/偏好胜率曲线归档；安全红队命中率下降趋势记录；模型卡更新。

## 第五篇 运维专家
- **推理部署（Ch11）：** 推荐自动化任务（如：`make infer-bench`）
  - 吞吐/延迟与量化精度对比表；限流、熔断、灰度发布日志。
- **LLMOps（Ch12）：** 推荐自动化任务（如：`make llmops-eval`）
  - 观测（LangSmith/日志）与评估（RAGAS/MT-Bench）报告；成本与缓存命中率趋势图。

---

## 发布与版本
- **版本标签：** 每轮重大更新建议创建 `book-vX.Y` 标签，并附变更日志与关键指标对比。
