...
## 关键流程图（纯文本）：预训练决策门禁（先便宜后训练）

```text
输入：能力缺口（可评测任务）+ 数据边界/许可 + 预算/风险
  ↓
0) 先写边界（止损线/回滚/守门指标）
  - 写不出来：停止（先补评测/回归/合规）
  ↓
1) 先跑便宜方案（RAG/提示/流程/评测回归）
  - 达标或接近达标：不做预训练
  - 明确不足：进入 2)
  ↓
2) 数据可用性审计（许可/脱敏/偏差/覆盖）
  - 不合规或覆盖不足：停止（先做数据与治理）
  ↓
3) 建立可复现基线（同口径回归集 + 基线报告）
  - 基线不稳定：停止（先把评测与门禁跑稳）
  ↓
4) 小跑试验（1% 数据/单卡/短时）
  - 趋势不明确或守门越界：止损 + 回滚到基线
  - 趋势明确：进入 5)
  ↓
5) 全量训练（每个 checkpoint 都跑对比报告）
  - 守门越界：立刻停训/回滚 + 触发样本回写数据与回归
  - 达到门槛：进入灰度发布
  ↓
6) 灰度发布
  - 线上退化越界：一键回滚到基线版本组合
...
输出（必须可审计）：决策卡 + 基线报告 + 对比报告 + 成本账本 + 回滚动作
```
...
## 示例（可复制）：预训练决策卡（可执行止损与回滚）

**目标：** 把预训练从“想做”变成“可裁决”：目标/基线/预算/守门/止损/回滚齐全，并能被脚本做最小校验。

**前置条件：**
- Python 3 可用

**步骤：**
1. 复制并运行下面脚本：它包含一份决策卡示例，并检查关键字段与阈值是否齐全。
```bash
python3 - <<'PY'
...
card_json = r'''
{
  "project": "contract_clause_extractor",
  "goal": "条款类型分类 F1 提升到 >=0.90（并减少误报）",
  "baseline": {"approach": "RAG + 规则抽取", "f1": 0.86},
  "data": {"dataset_snapshot_id": "snapshot_2025_12_01", "license_ok": true},
  "budget": {"gpu_hours_max": 20, "train_days_max": 3, "train_cost_usd_max": 800},
  "guardrails": {
    "pii_leak_rate_max": 0.001,
    "unauthorized_rate_max": 0.001,
    "p95_latency_increase_max": 0.10,
    "cost_increase_max": 0.10
  },
  "success_threshold": {"primary_f1_min": 0.90},
  "stop_loss": {
    "if_primary_f1_below": 0.88,
    "if_any_guardrail_exceeded": true
  },
  "rollback": {"to_model_id": "baseline_model_v1", "to_policy_id": "policy_v3"}
}
'''
...
assert card["data"]["license_ok"] is True
assert card["budget"]["gpu_hours_max"] > 0
assert 0 <= card["guardrails"]["pii_leak_rate_max"] <= 1
assert 0 <= card["guardrails"]["unauthorized_rate_max"] <= 1
assert 0 <= card["guardrails"]["p95_latency_increase_max"] <= 1
assert 0 <= card["guardrails"]["cost_increase_max"] <= 1
assert 0 <= card["success_threshold"]["primary_f1_min"] <= 1
assert 0 <= card["stop_loss"]["if_primary_f1_below"] <= 1

print("ok")
PY
```
2. 把决策卡与版本组合绑定：把 `dataset_snapshot_id`、评测集版本、训练配置与模型注册表版本写进同一个证据包目录。
3. 训练每个 checkpoint 都跑对比报告：主指标未达门槛或任一守门越界就触发止损，并自动回滚到 `baseline_model_v1/policy_v3`。

**验证命令：**
- 上面脚本输出 `ok` 且退出码为 0；并且你的对比报告能指向同一套版本组合（数据/评测/配置/模型）。

**失败判定：**
- 决策卡缺关键字段或阈值不可裁决（写成形容词）；基线不可复现；训练过程中守门越界但没有自动止损。

**回滚：**
- 停训并回滚到基线版本组合（模型/策略/索引/提示同步回退），保留对比报告与触发样本；把触发样本回写到守门回归与数据排除清单。
...
## 复现检查清单（本章最低门槛）
- 预训练决策卡已写：目标、预算、风险、成功门槛、止损线、回滚动作与证据路径齐全。[44]
- 评测基线可复现：不训练方案可运行、可对比，并能稳定产出同口径基线报告。[6]
- 止损线可执行：达不到门槛或守门越界时，停止训练或灰度并回滚到基线；同时固定证据包并启动复盘与排查。[44]

## 常见陷阱（失败样本）
1. **现象：** 花了大量预算，效果说不清；复盘时只有“感觉更好”。  
   **根因：** 没有可复现评测基线与成功门槛；训练变成许愿。[6]  
   **复现：** 让团队回答：基线指标是多少、训练后提升多少、门槛是多少、证据在哪；若只能给描述而给不出对比报告即命中。  
   **修复：** 先建立回归集与基线报告；训练过程每个 checkpoint 都跑对比评测并产出报告；达不到门槛立刻止损并回滚到基线。[6]  
   **回归验证：** 用固定回归集复跑两次，基线与训练后报告口径一致；门禁规则能自动判定通过/失败并产出证据包。

2. **现象：** 某些能力变好了，但通用能力退化或变危险（越权/注入/泄露）。  
   **根因：** 数据偏差与遗忘；缺乏守门指标与攻击集回归。[6]  
   **复现：** 训练后在注入/越权/PII 探测集上退化，或关键通用任务准确率下降，但训练流程没有阻断。  
   **修复：** 把注入、越权、PII 泄露与关键通用任务纳入守门集；任何守门退化就停训或回滚；触发样本回写数据排除清单与清洗规则。[6]  
   **回归验证：** 守门集版本化并进入发布门禁：任一守门越界即失败；修复后复跑能恢复到基线或更好。

3. **现象：** 训练做完却无法上线，运维与成本压垮团队；上线后单位成本越界。  
   **根因：** 只算训练成本，不算推理与维护成本；缺少灰度止损与降级策略。[44]  
   **复现：** 灰度后 p95 延迟与单位成本持续高于阈值，但没有回滚/降级路径；只能靠人工拍板。  
   **修复：** 在决策卡里写清推理成本上限与 SLA；灰度阶段用真实流量测算成本与延迟；长期成本越界优先做路由、量化、蒸馏或策略回退，而不是硬上预训练。[44]  
   **回归验证：** 灰度门禁可触发：指标越界→自动降级或回滚→成本/延迟回到基线；证据包可复盘。
