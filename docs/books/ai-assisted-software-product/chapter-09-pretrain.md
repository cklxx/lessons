...
**目标：** 在领域语料上做继续预训练（continued pretraining），并定义“可回滚”的评估门槛与日志归档，让“跑训练”变成可证伪的工程流程。[37][39]
...
- 若出现灾难性遗忘或领域指标回退，必须回滚到上一个 checkpoint/配置，而不是继续堆步数。[37]
...
**步骤：**
1. **准备语料**：去重、清洗、许可记录；输出语料规模、重复率与 token 估算。[34][35]
...
5. **归档与对比**：把日志、成本、能耗估计与评测表落盘；把最优 checkpoint 与决策理由写进报告。[39][44]
...
- **兼容性**：是否能复用旧 tokenizer（保留旧 token id）？如果不能，是否能接受下游全部重训？
- **OOV 率**：领域术语的 OOV 是否显著下降？（抽样域词表/关键词列表做对比）
- **压缩率**：平均 tokens/char 是否改善？（影响吞吐与成本）
- **特殊符号**：代码/公式/标点是否被合理切分？（避免把关键 token 切碎）
- **版本锁定**：tokenizer 文件（vocab/merges）是否版本化？是否写入 model card？

![图：词表变更的影响面（待补）](../../assets/fig-placeholder.svg)
...
一个最小“算账模板”（示意，目的是让你在开跑前就知道上限）：[39]
...
![图：成本/吞吐/显存三角关系（待补）](../../assets/fig-placeholder.svg)
...
## 常见陷阱
1. **现象：** 训练跑完了，但下游应用（embedding/RAG/推理）全部报错或质量崩。  
   **根因：** 词表不兼容：完全替换 tokenizer 导致 token id 语义变更，下游无法复用。[38]  
   **复现：** 用旧 tokenizer 编码一组典型输入，对比新 tokenizer 的 token 序列差异。  
   **修复：** 优先增量扩词表；若必须替换，明确“需要全链路重训”的成本与计划，并把兼容性测试纳入门禁。[38]  
   **回归验证：** tokenizer 变更必须产出 OOV/压缩率/兼容性报告，并能跑通关键下游回归用例。
...
3. **现象：** 成本与时间失控，中途停机，钱花了但产物不可复用。  
   **根因：** 未算账就开跑；日志/配置/数据版本缺失导致无法复现与对比。[39]  
   **复现：** 让团队回答“这次训练用了哪些数据/配置/预算/停止条件”，若答不上来就是不可审计。  
   **修复：** 训练前写成本表与停止条件；训练中实时记录吞吐/成本；训练后归档日志与 run_id。[39][44]  
   **回归验证：** `reports/pretrain_run.md` 包含：数据版本、配置、成本、能耗估计、评测表与决策结论。

4. **现象：** loss 看起来在降，但评测不提升甚至退化。  
   **根因：** 目标与评测不一致；语料噪声或分块策略导致模型学到无用模式。  
   **复现：** 抽样检查语料质量（重复/模板/垃圾文本），并对比不同语料比例的评测结果。  
   **修复：** 把评测集当裁判；清洗语料/调整采样；把“无提升即停止”写进门槛。  
