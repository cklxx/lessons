...
> 避免盲目“从零预训练”。多数场景你要的是：用更小的成本，把通用模型往你的领域推一推（继续/增量预训练）。[2][37][39]

!!! note "关于复现、目录与 CI"
    本章中出现的 `make ...`、持续集成（CI）、以及示例目录/文件路径（例如 `path/to/file`）均为工程约定，用于演示如何将方法应用到你自己的工程仓库中。本仓库仅提供文档，读者需自行实现或用等价工具链替代。

## 章节定位
本章解决“通用模型在特定领域知识深度或业务语境适配上存在不足”的问题。你将理解继续预训练的关键决策：语料怎么选、分词器怎么改、训练怎么控成本、评估怎么防止灾难性遗忘。[2][37][39]
...
2. **分词器策略**：优先采用“增量扩展词表”策略，慎用完全替换方案；输出 OOV 率与压缩率报告，并做兼容性检查。[38]
3. **训练配置**：小学习率、有限步数，写清：序列长度、batch、梯度累计、ZeRO/检查点、预算与停止条件（含熔断）。[39]
...
建立分词器变更的决策检查清单，避免词表变更导致全链路破坏：[38]
...
```text
追求目标：更长上下文（seq_len↑）  →  代价：显存↑、吞吐↓、P95↑
追求目标：更高吞吐（batch↑）      →  代价：显存↑、队列抖动↑
追求目标：更低显存（ZeRO/ckpt）    →  代价：工程复杂度↑、训练/推理开销↑
```
...
## 资料笔记（可选）
- [Datasheets（语料与许可记录怎么写清楚）](../../materials/ai-assisted-software-product/notes/ref-034-datasheets.md)
- [数据去重（继续预训练前的必做项之一）](../../materials/ai-assisted-software-product/notes/ref-035-llm-dedup.md)
