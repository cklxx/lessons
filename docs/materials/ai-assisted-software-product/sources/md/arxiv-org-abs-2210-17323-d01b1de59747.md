...
- PDF: https://arxiv.org/pdf/2210.17323.pdf
- Retrieved: 2025-12-17T17:00:42.981715+00:00
...
(/IgnoreMe)
(/) > [cs](https://arxiv.org/list/cs/recent) > arXiv:2210.17323 
...
Authors:[Elias Frantar](https://arxiv.org/search/cs?searchtype=author&query=Frantar,+E), [Saleh Ashkboos](https://arxiv.org/search/cs?searchtype=author&query=Ashkboos,+S), [Torsten Hoefler](https://arxiv.org/search/cs?searchtype=author&query=Hoefler,+T), [Dan Alistarh](https://arxiv.org/search/cs?searchtype=author&query=Alistarh,+D)
...
## Submission history
From: Elias Frantar [[view email](https://arxiv.org/show-email/e145ca9f/2210.17323)]   
**[[v1]](https://arxiv.org/abs/2210.17323v1)** Mon, 31 Oct 2022 13:42:40 UTC (154 KB)  
...
View a PDF of the paper titled GPTQ: Accurate Post-Training Quantization for Generative Pre-trained Transformers, by Elias Frantar and 3 other authors
  * [View PDF](https://arxiv.org/pdf/2210.17323)
  * [TeX Source ](https://arxiv.org/src/2210.17323)


[ view license ](http://creativecommons.org/licenses/by/4.0/ "Rights to this article")
Current browse context: 
cs.LG
[< prev](https://arxiv.org/prevnext?id=2210.17323&function=prev&context=cs.LG "previous in cs.LG \(accesskey p\)")   |   [next >](https://arxiv.org/prevnext?id=2210.17323&function=next&context=cs.LG "next in cs.LG \(accesskey n\)")   

[new](https://arxiv.org/list/cs.LG/new) |  [recent](https://arxiv.org/list/cs.LG/recent) | [2022-10](https://arxiv.org/list/cs.LG/2022-10)
Change to browse by: 
[cs](https://arxiv.org/abs/2210.17323?context=cs)  
...
### [ 2 blog links](https://arxiv.org/tb/2210.17323)
...
Data provided by: 
### Bookmark
(http://www.bibsonomy.org/BibtexHandler?requTask=upload&url=https://arxiv.org/abs/2210.17323&description=GPTQ: Accurate Post-Training Quantization for Generative Pre-trained Transformers "Bookmark on BibSonomy") (https://reddit.com/submit?url=https://arxiv.org/abs/2210.17323&title=GPTQ: Accurate Post-Training Quantization for Generative Pre-trained Transformers "Bookmark on Reddit")
Bibliographic Tools
...
[Which authors of this paper are endorsers?](https://arxiv.org/auth/show-endorsers/2210.17323) | [Disable MathJax](javascript:setMathjaxCookie\(\)) ([What is MathJax?](https://info.arxiv.org/help/mathjax.html)) 
  * [About](https://info.arxiv.org/about)
...
```text
                                         ...
                                         FOR G ENERATIVE P RE - TRAINED T RANSFORMERS
                                          ...
                                          IST Austria             ETH Zurich             ETH Zurich             IST Austria & NeuralMagic
...
whether one-shot post-training quantization to higher compression rates is generally-feasible.
                                  ...
                                  50
                                                          OPT Model Family                                                    BLOOM Model Family
                                  45                                                                               60
                                                                                 110.                                                                         571.
                                  40                                                                               50
                                  ...
                                  10          4bit GPTQ
                                   ...
                                   5
                                       10 1               100           101        102                                  100           101               102
                                                           #params in billions                                                  #params in billions
...
Then, the objective is to find a matrix of quantized weights W  c which minimizes the squared error,
...
whose Hessian is HF = 2XF X>      F , where F denotes the set of remaining full-precision weights,
...
tized. This is done efficiently, avoiding expensive full recomputations of H−1 , by removing the qth
...
The original OBQ method quantizes rows of W                     Inverse Layer Hessian
                                                                                              Weight Matrix / Block
independently, in a specific order defined by the                  (Cholesky Form)
...
H−1F is always the same for all rows (see Fig-
ure 2 for an illustration). In more detail, the lat-
ter is due to the fact that HF depends only on                                                block i quantized recursively
                                                                    computed initially
                                                                                                   column-by-column
the layer inputs XF , which are the same for all
rows, and not on any weights. Therefore, we                                                       unquantized weights
                                                                          quantized weights
have to perform the update of H−1         F given by
                                                                                                    that are updated

Equation (3) only dcol times, once per column,
rather than drow ·dcol times, once per weight. This           Figure 2: GPTQ quantization procedure. Blocks
reduces the overall runtime from O(drow · d3col )             of consecutive columns (bolded) are quantized at
to O(max {drow · d2col , d3col }), i.e., by a factor of       a given step, using the inverse Hessian informa-
min {drow , dcol }. For larger models, this differ-           tion stored in the Cholesky decomposition, and
ence consists of several orders of magnitude.                 the remaining weights (blue) are updated at the
However, before this algorithm can actually be                end of the step. The quantization procedure is
applied to very large models in practice, two ad-             applied recursively inside each block: the white
ditional major problems need to be addressed.                 middle column is currently being quantized.
...
(3) given below, with Q denoting a set of indices, and H−1 −Q denoting the inverse matrix with the
...
matrix H−1F becomes indefinite, which we notice can cause the algorithm to aggressively update the
...
To address this, we begin by noting that the only information required from H−1 Fq , where Fq denotes
...
Algorithm 1 Quantize W given inverse Hessian H−1 = (2XX> + λI)−1 and blocksize B.
  ...
  H−1 ← Cholesky(H−1 )>                                    // Hessian inverse information
  for i = 0, B, 2B, . . . do
     for j = i, . . . , i + B − 1 do
        Q:,j ← quant(W:,j )                                // quantize column
        E:,j−i ← (W:,j − Q:,j ) / [H−1 ]jj                 // quantization error
...
Published as a conference paper at ICLR 2023
  ...
  AdaQuant      68.12      59.21       74.68     64.98               Runtime        20.9m     44.9m        1.6h      4.2h
  BRECQ         69.37      68.47       75.88     75.32               BLOOM          1.7B       3B          7.1B     176B
...
Table 1: Comparison with state-of-the-art                          of the 4 largest OPT and BLOOM models.
...
Published as a conference paper at ICLR 2023
                                                ...
                                                FP16          4bit GPTQ            4bit RTN                               3bit GPTQ            3bit RTN

                            0.8
                                                  OPT Family                                                                   BLOOM Family
...
zation grid. For example, it is easily combined with standard grouping (Alistarh et al., 2017; Park
...
at 4-bit precision.
                                                                                         ...
                                                                                         Perplexity on WikiText2
                                                                                                                   ...
                                                                                                                   14
    Model                             FP16   g128      g64     g32        3-bit                                    13

    OPT-175B                          8.34   9.58      9.18    8.94       8.68                                     12
    BLOOM                             8.11   9.55      9.17    8.83       8.64                                     11
                                                                                                                   10
Table 7: 2-bit GPTQ quantization results with
                                                                                                                    9 0
varying group-sizes; perplexity on WikiText2.                                                                       10                                      101
                                                                                                                                         #params in billions
...
A       A PPENDIX
A.1      A DDITIONAL C OMPARISON WITH OBQ
...
A.2.2      T IMING E XPERIMENT S ETUP
...
A.3      A DDITIONAL L ANGUAGE G ENERATION R ESULTS
...
A.4       A DDITIONAL Z ERO S HOT R ESULTS
