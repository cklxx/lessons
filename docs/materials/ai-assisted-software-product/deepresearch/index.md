...
- [[83] AI 原型工具（NN/G）：远看还行，近看稀碎的风险与用法](ref-083-ai-prototyping.md)（9.55） · UX / UI & Design Systems
- [[72] People + AI Guidebook（Google PAIR）：把以人为本写成可执行的工作流](ref-072-pair-guidebook.md)（9.40） · UX / UI & Design Systems
- [[80] Customer-Service Chat UX（NN/G）：把对话做成可恢复的闭环](ref-080-chat-ux.md)（9.40） · UX / UI & Design Systems
- [[81] Prompt Controls（NN/G）：把可控性做成界面，而不是藏在提示词里](ref-081-prompt-controls-genai.md)（9.40） · UX / UI & Design Systems
- [[82] Testing AI with Real Design Scenarios（NN/G）：用真实场景给 AI 体验写验收](ref-082-testing-ai-methodology.md)（9.40） · UX / UI & Design Systems
- [[84] 用 AI 造测试数据（NN/G）：用假表格和图表把原型走查变真实](ref-084-ai-data-prototype-testing.md)（9.40） · UX / UI & Design Systems
