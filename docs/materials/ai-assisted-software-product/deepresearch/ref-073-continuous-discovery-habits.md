# Deep Research: [73] Continuous Discovery Habits（Teresa Torres）：把需求挖掘变成可持续的习惯
...
## TL;DR
持续探索（Continuous Discovery）并非指永无止境的研究，而是由产品、设计、工程组成的铁三角建立起每周接触用户的**节奏性习惯**，通过机会解决方案树（OST）将业务目标拆解为可验证的假设，从而基于证据而非直觉做出产品决策。
